...
class RedisTemplateManagerTest {
    ...
    public void 레디스데이터_적재_성공() throws JsonProcessingException {
        ...
        // when
        redisTemplateManager.saveProductByMallName(mallName, scheduledRedisProductRequest);

        // then
        verify(redisJsonTemplate.opsForValue(), times(1)).set(mallName, serializedString);
    }
    ...
    public void 레디스데이터_추출() throws JsonProcessingException {
        ...
        // when
        Optional<? extends RedisProduct<?>> result =  redisTemplateManager.getProductsByMallName(mallName);

        // then
        verify(redisJsonTemplate.opsForValue(), times(1)).get(mallName);
        result.ifPresent(redisProduct -> assertThat(redisProduct.getMall_name()).isEqualTo(mapProductResponse.getMall_name()));
    }
}